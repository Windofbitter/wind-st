...
- Routing map (see `src/App.tsx`): `/chat`, `/characters`, `/characters/:characterId`, `/user-personas`, `/presets`, `/lorebooks`, `/lorebooks/:lorebookId`, `/llm-connections`, `/mcp-servers`, `/runs`, `/health`, `/settings`. `/` redirects to `/chat`.
