...
- Optional ports: `dev.bat <backendPort> <frontendPort>` (defaults 3000 / 5173). The script points the dev proxy (`VITE_API_PROXY_TARGET`) to the backend port.
...
- 可选端口：`dev.bat <backendPort> <frontendPort>`（默认为 3000 / 5173）。脚本会将 dev 代理（`VITE_API_PROXY_TARGET`）指向对应后端端口。
